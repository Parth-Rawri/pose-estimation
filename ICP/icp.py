...
from utils import depth_to_point_cloud, transform_point3s, Camera, cam_view2pose
...
def obj_depth2pts(obj_id, depth, mask, camera, view_matrix):
    ...
    """
    object_depth_image = gen_obj_depth(obj_id, depth, mask)
    camera_instrinsic = camera.intrinsic_matrix
    object_point_cloud = depth_to_point_cloud(camera_instrinsic, object_depth_image)
    camera_pose = cam_view2pose(view_matrix)
    world_pts = transform_point3s(camera_pose, object_point_cloud)
    return world_pts
...
def align_pts(pts_a, pts_b, max_iterations=20, threshold=1e-05):
    ...
    init_matrix, transformed, cost = trimesh.registration.procrustes(pts_a,
                                                                     ...
                                                                     scale=False,
                                                                     return_cost=True)
    matrix, _, _ = trimesh.registration.icp(pts_a, pts_b, init_matrix, threshold, max_iterations)
    return matrix
...
def estimate_pose(depth, mask, camera, view_matrix):
    ...
    for obj_id in range(1, 6):  
        ...
        pts_obj = obj_depth2pts(obj_id, depth, mask, camera, view_matrix)
        if len(pts_obj) == 0: # object mask is empty for this object
            ...
            pts_mesh = obj_mesh2pts(LIST_OBJ_FOLDERNAME[obj_id - 1], point_num=len(pts_obj))
            # TODO: compute relative transform between pts_obj and pts_mesh
            transform = align_pts(pts_mesh, pts_obj)
...
def export_gt_ply(scene_id, depth, gt_mask, camera, view_matrix):
    ...
    file_path = "../segmentation/dataset/val/exported_ply/" + str(scene_id) + "_gtmask.ply"
...
def main():
    ...
    if args.val:
        dataset_dir = "../segmentation/dataset/val/"
        print("Pose estimation for validation set")
    elif args.test:
        dataset_dir = "../segmentation/dataset/test/"
    ...
    for scene_id in range(5):
        ...
        depth = cv2.imread(os.path.join(dataset_dir, 'depth', f'{scene_id}_depth.png'), -1).astype(float)/1000
        pred_mask = cv2.imread(os.path.join(dataset_dir, 'pred', f'{scene_id}_pred.png'), -1)
        ...
        if args.val:
            gt_mask = cv2.imread(os.path.join(dataset_dir, 'gt', f'{scene_id}_gt.png'), -1)
